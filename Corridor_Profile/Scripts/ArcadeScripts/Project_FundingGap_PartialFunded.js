...
if (HasKey($dataSources, fullLayerID)) {
    ...
    if (Count(selectedFeatures) > 0) {
      ...
      // Try to get Project_FundingGap_PartialFunded directly first
      var value = feature["Project_FundingGap_PartialFunded"];
        ...
        if (!IsEmpty(targetMiles)) {
            ...
            for (var f in allFeats) {
                if (Abs(f["Total_Miles"] - targetMiles) < 0.01) {
                    value = f["Project_FundingGap_PartialFunded"];
                    break; // Found it
                }
            }
        }
      ...
      // Display the Project_FundingGap_PartialFunded for the selected feature
      if (!IsEmpty(value)) {
      ...
      // CASE B: Nothing selected -> Sum total Project_FundingGap_PartialFunded
      var allFeatures = fullSource.layer;
      var totalValue = Sum(allFeatures, "Project_FundingGap_PartialFunded");
