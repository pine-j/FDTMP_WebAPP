...
if (HasKey($dataSources, fullLayerID)) {
    ...
    if (Count(selectedFeatures) > 0) {
      ...
      // Try to get Projects_PartialFunded directly first
      var value = feature["Projects_PartialFunded"];
        ...
        if (!IsEmpty(targetMiles)) {
            ...
            for (var f in allFeats) {
                if (Abs(f["Total_Miles"] - targetMiles) < 0.01) {
                    value = f["Projects_PartialFunded"];
                    break; // Found it
                }
            }
        }
      ...
      // Display the Projects_PartialFunded for the selected feature
      if (!IsEmpty(value)) {
      ...
      // CASE B: Nothing selected -> Sum total Projects_PartialFunded
      var allFeatures = fullSource.layer;
      var totalValue = Sum(allFeatures, "Projects_PartialFunded");
