...
if (HasKey($dataSources, fullLayerID)) {
    ...
    if (Count(selectedFeatures) > 0) {
      ...
      for (var selFeature in selectedFeatures) {
          ...
          var val = GetVal(selFeature, "four_U_plus_miles");
                  ...
                  for (var f in allFeats) {
                      ...
                      if (!IsEmpty(fHWY)) {
                          if (Lower(Trim(Text(fHWY))) == targetKey) {
                              val = GetVal(f, "four_U_plus_miles");
                              foundMatch = true;
                              break; 
                          }
                      }
          ...
          if (!IsEmpty(val)) {
              miles4Uplus += Number(val);
          }
      }
      
      displayText = Text(miles4Uplus, "#,###.0") + " mi";
    } else {
      // CASE B: Nothing selected -> Sum total 4U+ miles
      var allFeatures = fullSource.layer;
      var total4UplusMiles = Sum(allFeatures, "four_U_plus_miles");
      
      displayText = Text(total4UplusMiles, "#,###.0") + " mi";
    }
} else {
