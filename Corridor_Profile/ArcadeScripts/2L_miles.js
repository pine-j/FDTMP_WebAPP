...
if (HasKey($dataSources, fullLayerID)) {
    ...
    if (Count(selectedFeatures) > 0) {
      ...
      for (var selFeature in selectedFeatures) {
          ...
          var val = GetVal(selFeature, "two_L_miles");
      ...
      displayText = Text(miles2L, "#,###.0") + " mi";
    } else {
      // CASE B: Nothing selected -> Sum total 2L miles
      var allFeatures = fullSource.layer;
      var total2LMiles = Sum(allFeatures, "two_L_miles");
      
      displayText = Text(total2LMiles, "#,###.0") + " mi";
    }
} else {
